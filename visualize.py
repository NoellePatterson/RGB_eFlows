...
def overlap_plots(rh_nat, rh_obs):
    ...
    for index, site in enumerate(rh_nat):
        ...
        def plot_stuff(plotlines, color, plot_label):
            ...
            plt.fill_between(x, plotlines['twenty_five'], plotlines['fifty'], color=color, alpha = 0.6)
            plt.fill_between(x, plotlines['fifty'], plotlines['seventy_five'], color=color, alpha = 0.6)
        plot = plot_stuff(plotlines_nat, '#3d49f5', 'Naturalized')
        plot = plot_stuff(plotlines_obs, '#D6001C', 'Observed')
        ...
        plt.title(site['name'])  
        plt.savefig('data_outputs/RGB_overlaid_rh/'+site['name']+'.png')  
        plt.clf()
        # import pdb; pdb.set_trace()
